...
def extract_panel_coordinates(image: np.ndarray, panel_area_threshold: int = 1000) -> List[Tuple]:
...
def preprocess_images(file_paths: List[str], apply_prog_bar: bool = False, apply_panel_extraction: bool = False):
...
def download():
    ...
    apply_panel_extraction = input("Do you want to apply experimental panel extraction on all pages? [y/n] ").lower() == "y"
    ...
    if not compile_into_single:
        ...
        for chapter in tqdm(chapters):
            ...
            preprocess_images(image_files, apply_panel_extraction=apply_panel_extraction)
            if apply_panel_extraction:
                image_files = sorted(
        ...
        preprocess_images(all_image_files, apply_prog_bar=True, apply_panel_extraction=apply_panel_extraction)
        print("Compiling PDF...")
        pdf_path = os.path.join(pdf_location, f"{comic_name}.pdf") if not apply_panel_extraction else os.path.join(pdf_location, f"{comic_name} (panelled).pdf")
        if apply_panel_extraction:
            all_image_files = []
            for chapter in chapters:
                image_files = sorted(
